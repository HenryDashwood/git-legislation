...
Known enacted source and point-in-time snapshots:
...
git-legislation convert var/cache/point-in-time/2026-05-03/ukpga/2026/14.xml
...
### Milestone 3: Enacted Corpus Fetching

- Keep `ukpga` as the first complete path.
- Add a supported legislation type registry.
- Walk year feeds for supported legislation types.
- Fetch enacted XML into `output/xml/enacted/{type}/{year}/{number}.xml`.
- Write a manifest of fetched, skipped, missing, and failed documents.

### Milestone 4: Point-In-Time Corpus Fetching

- Accept an explicit snapshot date.
- Discover documents that existed by that date.
- Fetch dated XML into `output/xml/point-in-time/{yyyy-mm-dd}/{type}/{year}/{number}.xml`.
- Treat unavailable point-in-time XML as expected.
- Write a dated manifest of fetched, skipped, missing, and failed documents.

### Milestone 5: Representative Documents
